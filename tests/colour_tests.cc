...
TEST(ColourTests, ColourOperations){
    ...
    EXPECT_TRUE((a + b).isEqual(Colour(1.6, 0.7, 1.0)));
    EXPECT_TRUE((a - b).isEqual(Colour(0.2, 0.5, 0.5)));

    Colour c(0.2, 0.3, 0.4);
    EXPECT_TRUE((c * 2).isEqual(Colour(0.4, 0.6, 0.8)));

    a = Colour(1, 0.2, 0.4);
    b = Colour(0.9, 1, 0.1);
    EXPECT_TRUE((a * b).isEqual(Colour(0.9, 0.2, 0.04)));
}