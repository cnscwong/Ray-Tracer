...
TEST(RayTest, IntersectionTest){
    ...
    Sphere s;
    std::vector<Intersection> intersections = r.RaySphereIntersection(s);
    EXPECT_EQ(intersections.size(), 2);
    EXPECT_TRUE(floatIsEqual(intersections.at(0).getTime(), 4.0));
    EXPECT_TRUE(floatIsEqual(intersections.at(1).getTime(), 6.0));
    EXPECT_TRUE(intersections.at(0).getSphere().isEqual(s));
    EXPECT_TRUE(intersections.at(1).getSphere().isEqual(s));
    
    // Ray intersects the sphere at one point
    r = Ray(Point(0, 1, -5), Vector(0, 0, 1));
    intersections = r.RaySphereIntersection(s);
    EXPECT_EQ(intersections.size(), 2);
